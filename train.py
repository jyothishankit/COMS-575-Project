...
class Trainer:
    ...
    def train_step(self, batch):
        ...
        one_labels = torch.ones(BATCH_SIZE_CONSTANT).to(self.gpu_id)
    ...
    def train(self):
        scheduler_G = torch.optim.lr_scheduler.StepLR(
            self.optimizer, step_size=DECAY_EPOCH_INTERVAL_CONSTANT, gamma=0.5
        )
        scheduler_D = torch.optim.lr_scheduler.StepLR(
            self.optimizer_disc, step_size=DECAY_EPOCH_INTERVAL_CONSTANT, gamma=0.5
        )
